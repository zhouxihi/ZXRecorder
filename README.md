...
- (void)startAction {
    ...
    [[ZXRecorder shareInstance] setMetering:YES];
    [[ZXRecorder shareInstance] PrepareWithFileName:@"123.caf" formate:kAudioFormatMPEG4AAC];
    [[ZXRecorder shareInstance] record];
    
    self.recording = YES;
    
    self.link = [CADisplayLink displayLinkWithTarget:self selector:@selector(printAveragePower)];
...
- (void)playAction {
    ...
    [[ZXSingletonPlayer shareInstance] setSourceFromFile:[[ZXRecorder shareInstance] getAudioFileWithName:@"123.caf"] error:nil];
    [[ZXSingletonPlayer shareInstance] play];
}

- (void)printAveragePower {
    
    NSLog(@"averagePower: %f", [[ZXRecorder shareInstance] getAveragePowerForChannel]);
}

- (void)printPeakPower {
    
    NSLog(@"prakPower: %f", [[ZXRecorder shareInstance] getPeakPowerForChannel]);
}
```
